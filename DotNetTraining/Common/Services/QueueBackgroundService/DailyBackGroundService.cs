...
public class DailyTaskService : BackgroundService
{
    ...
    private readonly TimeSpan _targetTime = new TimeSpan(10, 40, 0); // 10:40 AM

    public DailyTaskService(IServiceProvider serviceProvider, ILogManager logger)
    {
        _serviceProvider = serviceProvider;
        _logger = logger;
        Directory.CreateDirectory("Logs"); // để SeriLog có nơi ghi nếu chưa có
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            _logger.Info($"Next run scheduled for: {nextRun} (UTC: {nextRunInUtc})");
            _logger.Info($"Calculated delay: {delay.TotalSeconds} seconds");
    ...
    private async Task RunDailyTaskAsync(CancellationToken stoppingToken)
    {
        using (var scope = _serviceProvider.CreateScope())
        {
            ...
            var emailService = scope.ServiceProvider.GetRequiredService<IEmailLogService>();
            ...
            try
            {
                ...
                _logger.Info($"[DailyTaskService] Task is running at {DateTime.Now}");
                await emailService.SendReminderEmailAsync();
                _logger.Info($"[DailyTaskService] Task completed at {DateTime.Now}");
            }
            catch (Exception ex)
            {
